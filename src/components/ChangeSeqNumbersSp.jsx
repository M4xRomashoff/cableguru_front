...
function getCableList(l, cables, cableListRaw, header) {
  ...
  cableList.map((item, index) => {
    ...
    item['seq'] = header[6][index + 1].toString();
  });

  return cableList;
}

const ChangeSeqNumbersSp = ({l, dataFcs, changeSeqPoint, cables, getSpDataRequest, onClose, connections }) => {
  ...
  async function onSaveChanges() {
    ...
    const response2 = await updateSeqNumbers('sp', changeSeqPoint.id, cablesList);
    getSpDataRequest();
    onClose();
  }
  function keyGen() {

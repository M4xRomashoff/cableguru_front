...
import { updateConnectionsAdd, updateConnectionsRemove, updateSplicesSp } from '../api/dataBasesApi';
...
function getCableListRaw(connections, id) {
  ...
  connections.map((item) => {
    if (item?.sp_id === id) cableList.push(item);
  });
  return cableList;
}
function getCableList(cables, cableListRaw, header) {
  ...
  cableList.map((item, index) => {
    item.name_id += header[3][index + 1];
  });

  return cableList;
}
const SpliceFibers = ({ dataFcs, updateState, spliceFibersPoint, cables, getSpDataRequest, onClose, connections }) => {
  ...
  useEffect(() => {
    const cableListRaw = getCableListRaw(connections, spliceFibersPoint.id);
  ...
  async function onSaveChanges() {
    if (cable1.id && cable2.id) {
      if (parseInt(endFiber1) <= cable1.size) {
        if (parseInt(endFiber2) <= cable2.size) {
          ...
          const res = await updateSplicesSp(data);
    ...
    updateState('sp', spliceFibersPoint.id, 2);
    const res = await logAddInfo(spliceFibersPoint.name_id, 'Fiber Assignment Changed', 'State changed to "Need to splice"');
    const itemUpdate = changeItemState(spliceFibersPoint.id, spliceFibersPoint.name_id, 'sp', 2);
